...
#Set parameters -outdated

# deprecated: these are now defined in main as globals
# frame_length = 512 
#Number of bins in each window (halved eventually). keep above 500 and even otherwise bin ranges are too small
...
def entire_utterance(speech): #Windows signal (Using above parameters) in overlapped steps (to proerly use hamming) and calls the fft function above to find the magnitude and power of the signal
    ...
    #print(num_samples,"frame_length",frame_length,step_length)
...
def save_to_file(file_path, file_name, mfcc): #Let's us save proccessed files to speed up using data for DNN
...
def main(filter_type, filter_shape, overlaps, frame_sizes, energy_included, temporal_included, no_bins, truncate_no_bins, file_dir, file_name):
    global step_length
    global frame_length
    ...
    file_path += "_ol_" + str(overlaps).replace(".","_") + "_fr_" + str(frame_sizes)+"_en_"+ str(energy_included)[0]+ "_temp_"+ str(temporal_included)[0]+ "_bins_"+ str(no_bins)+ "_trunc_bin_"+ str(truncate_no_bins)
    frame_length = frame_sizes                                              #Number of bins in each window (halved eventually). keep above 500 and even otherwise bin ranges are too small
    over_lap = overlaps                                                  #Sets the overlap of our windows
    step_length = int(frame_length * (over_lap))
    #print(frame_length,step_length)
    filter_number = no_bins 
    ...
    if filter_type == 'Mel':
        ...
        filter_bank = plot_barkscale(sample_frequency)
    ...
    keep_number = no_bins - truncate_no_bins
   ...
   # dft_mfccs -= np.mean(dft_mfccs, axis=0)
    ...
    save_to_file(file_path, file_name, dft_mfccs)
...
#Def function
def extract_features(filter_type, filter_shape, overlap, frame_sizes, energy_included, temporal_included, no_bins, truncate_no_bins):
    for audio_file in sorted(glob.glob('Recordings/*.wav')):
        ...
        main(filter_type, filter_shape, overlap, frame_sizes, energy_included, temporal_included, no_bins, truncate_no_bins, audio_file, audio_file_name)
